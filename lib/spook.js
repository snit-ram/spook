...
function casperCmds(argv, cb) {
  ...
  async.parallel({
    ...
    var name;
    res.tests.forEach(function(test, index) {
      name = test;
      if (test.substr(-3) === '.js') {
        name = test.substr(0, test.length - 3);
      }
      
      var xunitArg = argv.xunit ? (['--xunit=' + argv.xunit + '.' + index]) : [];
      
      cmds[name] = {
        // slimerjs chokes if relative paths to tests do not start './'
        cmd: ['test'].concat(['./' + path.join(argv.base, test)]).concat(args).concat(xunitArg),
        base: argv.base
      };
    });
